...
namespace GW2DotNET.V1.Common.Converters
{
    ...
    public sealed class UnknownGuildConverter : JsonConverter
    {
        ...
        {
            return objectType == typeof(Guild);
        }
        ...
        {
            var guildId = serializer.Deserialize<Guid?>(reader);
            if (!guildId.HasValue)
            {
                return null;
            }

            return new Guild { GuildId = guildId.Value };
        }
        ...
        {
            serializer.Serialize(writer, ((Guild)value).GuildId);
        }
    }
}