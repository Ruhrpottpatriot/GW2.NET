// --------------------------------------------------------------------------------------------------------------------
// <copyright file="UnknownMapConverter.cs" company="GW2.NET Coding Team">
...
namespace GW2DotNET.V1.Common.Converters
{
    ...
    public sealed class UnknownMapConverter : JsonConverter
    {
        ...
        {
            var mapId = serializer.Deserialize<int?>(reader);
            if (!mapId.HasValue)
            {
                return null;
            }

            return new Map { MapId = mapId.Value };
        }
        ...
        {
            serializer.Serialize(writer, ((Map)value).MapId);
        }
    }
}