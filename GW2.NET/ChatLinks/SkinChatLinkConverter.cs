...
namespace GW2DotNET.ChatLinks
{
    ...
    internal class SkinChatLinkConverter : ChatLinkConverter<SkinChatLink>
    {
        ...
        protected override SkinChatLink ConvertFromBytes(byte[] bytes)
        {
            return new SkinChatLink { SkinId = BitConverter.ToInt32(bytes, 0) };
        }

        /// <summary>Converts the given chat link to a byte array.</summary>
        /// <param name="value">The chat link.</param>
        /// <returns>A byte array.</returns>
        protected override byte[] ConvertToBytes(SkinChatLink value)
        {
            return BitConverter.GetBytes(value.SkinId);
        }
    }
}